...
class ReviewEndpoint extends Endpoint {
  ...
  Future<Review> update(Session session, {required String token, required int reviewId, int? rating, String? comment}) async {
    final user = await AuthGuard.requireUser(session, token);
    return _reviews.update(session, userId: user.id!, reviewId: reviewId, rating: rating, comment: comment);
  }

  Future<void> delete(Session session, {required String token, required int reviewId}) async {
    final user = await AuthGuard.requireUser(session, token);
