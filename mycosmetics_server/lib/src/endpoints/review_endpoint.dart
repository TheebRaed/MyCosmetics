...
class CouponEndpoint extends Endpoint {
  ...
  Future<CouponValidationResult> validate(Session session, {required String token, required String code}) async {
  ...
  Future<CouponValidationResult> apply(Session session, {required String token, required String code}) async {
    final user = await AuthGuard.requireUser(session, token);
    final cartSummary = await _cart.getCart(session, user.id!);
    return _coupons.apply(session, userId: user.id!, code: code, currentSubtotal: cartSummary.subtotal);
  }

  Future<void> remove(Session session, {required String token}) async {
    final user = await AuthGuard.requireUser(session, token);
  ...
  Future<Coupon> setActive(Session session, {required String token, required int id, required bool isActive}) async {
    await AuthGuard.requireAdminOrStaff(session, token);
