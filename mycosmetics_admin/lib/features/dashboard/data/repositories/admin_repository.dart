...
class AdminRepository {
  ...
  Future<void> updateOrderStatus(int orderId, String newStatus, {String? note}) =>
      _api.post(endpoint: 'order', method: 'updateStatus', body: {'orderId':orderId,'newStatus':newStatus,if(note!=null)'note':note}, fromJson: (_) {});
  ...
  Future<void> suspendUser(int userId, String reason) =>
      _api.post(endpoint: 'admin', method: 'suspendUser', body: {'userId':userId,'reason':reason}, fromJson: (_) {});

  Future<void> reactivateUser(int userId) =>
      _api.post(endpoint: 'admin', method: 'reactivateUser', body: {'userId':userId}, fromJson: (_) {});
  ...
  Future<void> adjustStock(int variantId, int newQty, String reason) =>
      _api.post(endpoint: 'admin', method: 'adjustStock', body: {'variantId':variantId,'newQty':newQty,'reason':reason}, fromJson: (_) {});
  ...
  Future<void> createCoupon({required String code, required String type, required double value, double minSpend=0, double? maxDiscount, int? usageLimit, String? expiresAt}) =>
      _api.post(endpoint: 'coupon', method: 'create', body: {'code':code,'type':type,'value':value,'minSpend':minSpend,if(maxDiscount!=null)'maxDiscount':maxDiscount,if(usageLimit!=null)'usageLimit':usageLimit,if(expiresAt!=null)'expiresAt':expiresAt}, fromJson: (_) {});

  Future<void> setCouponActive(int id, bool isActive) =>
      _api.post(endpoint: 'coupon', method: 'setActive', body: {'id':id,'isActive':isActive}, fromJson: (_) {});
  ...
  Future<void> createNotification({required String title, required String body, required String audience, String? audienceFilter, String? scheduledAt}) =>
      _api.post(endpoint: 'admin', method: 'createNotification', body: {'title':title,'body':body,'audience':audience,if(audienceFilter!=null)'audienceFilter':audienceFilter,if(scheduledAt!=null)'scheduledAt':scheduledAt}, fromJson: (_) {});
